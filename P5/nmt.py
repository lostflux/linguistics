...
#=======================================================================
# LING48/CS72 Homework 5: Attention-Based Neural Machine Translation
...
def save_loss_plot(train_losses, val_losses, opts):
    ...
    plt.title('BS={}, nhid={}'.format(opts.batch_size, opts.hidden_size),
              fontsize=20)
...
def load_data():
    """Loads (English, Pig-Latin) word pairs, and creates mappings from
    ...
    # Create the inverse mapping, from indexes to characters (used to decode the
    # model's predictions)
...
def create_dict(pairs):
    ...
    (source, target) pairs]}. This is used to make batches: each batch consists
    of two parallel tensors, one containing all source indexes and the other
    containing all corresponding target indexes. Within a batch, all the source
    words are the same length, and all the target words are the same length.
    """
...
def translate_sentence(sentence, encoder, decoder, idx_dict, opts):
    """Translates a sentence from English to Pig-Latin, by splitting the
    sentence into words (whitespace-separated), running the encoder-decoder
    model to translate each word independently, and then stitching the words
    back together with spaces between them.
    """
    if idx_dict is None:
      line_pairs, vocab_size, idx_dict = load_data()
    return ' '.join([translate(word, encoder, decoder, idx_dict, opts) for word
                     in sentence.split()])
...
def translate(input_string, encoder, decoder, idx_dict, opts):
    ...
    # For BS = 1
    decoder_input = to_var(torch.LongTensor([[start_token]]), opts.cuda)
    decoder_inputs = decoder_input

    for i in range(max_generated_chars):
      # slow decoding, recompute everything at each time
      decoder_outputs, attention_weights = decoder(decoder_inputs,
                                                   encoder_annotations,
                                                   decoder_hidden)
      ...
      if ni == end_token:
          ...
          gen_string = "".join([index_to_char[int(item)] for item in
                                generated_words.reshape(-1)])
...
def compute_loss(data_dict, encoder, decoder, idx_dict, criterion, optimizer,
                 opts):
    """Train/Evaluate the model on a dataset.

    Arguments:
        data_dict: The validation/test word pairs, organized by source and
        target lengths.
        encoder: An encoder model to produce annotations for each step of the
        input sequence.
        decoder: A decoder model (with or without attention) to generate output
        tokens.
        idx_dict: Contains char-to-index and index-to-char mappings, and start &
        end token indexes.
        criterion: Used to compute the CrossEntropyLoss for each decoder output.
        optimizer: Train the weights if an optimizer is given. None if only
        evaluate the model.
    ...
    for key in data_dict:
        ...
        input_tensors = [torch.LongTensor(string_to_index_list(s, char_to_index,
                                                               end_token))
                         for s in input_strings]
        target_tensors = [torch.LongTensor(string_to_index_list(s,char_to_index,
                                                                end_token))
        ...
        for i in range(num_batches):
            ...
            # The last hidden state of the encoder becomes the first hidden
...
def training_loop(train_dict, val_dict, idx_dict, encoder, decoder, criterion,
                  optimizer, opts):
    """Runs the main training loop; evaluates the model on the val set every
       epoch.
    ...
    Arguments:
        train_dict: The training word pairs, organized by source and target
        lengths.
        val_dict: The validation word pairs, organized by source and target
        lengths.
        idx_dict: Contains char-to-index and index-to-char mappings, and start &
        end token indexes.
        encoder: An encoder model to produce annotations for each step of the
        ...
        tokens.
        criterion: Used to compute the CrossEntropyLoss for each decoder output.
        optimizer: Implements a step rule to update the parameters of the
        encoder and decoder.
        opts: The command-line arguments.
    """
    start_token = idx_dict['start_token']
    ...
    for epoch in range(opts.nepochs):
        ...
        train_loss = compute_loss(train_dict, encoder, decoder, idx_dict,
                                  criterion, optimizer, opts)
        val_loss = compute_loss(val_dict, encoder, decoder, idx_dict, criterion,
                                None, opts)

        if val_loss < best_val_loss:
            checkpoint(encoder, decoder, idx_dict, opts)

        gen_string = translate_sentence(TEST_SENTENCE, encoder, decoder,
                                        idx_dict, opts)
        print(
            "Epoch: {:3d} | Train loss: {:.3f} | Val loss: {:.3f} | Gen: {:20s}"
            .format(epoch + 1, train_loss, val_loss, gen_string))
...
def train(opts):
    ...
    # Group the data by the lengths of the source and target words, to form
    # batches
    ...
    ##########################################################################
    encoder = GRUEncoder(vocab_size=vocab_size, hidden_size=opts.hidden_size,
                         opts=opts)
...
class GRUEncoder(nn.Module):
    ...
    def forward(self, inputs):
        ...
        Arguments:
            inputs: Input token indexes across a batch for all time steps in the
            sequence. (batch_size x seq_len)

        Returns:
            annotations: The hidden states computed at each step of the input
            sequence. (batch_size x seq_len x hidden_size)
            hidden: The final hidden state of the encoder, for each sequence in
            a batch. (batch_size x hidden_size)
        """
    ...
    def init_hidden(self, bs):
        """Creates a tensor of zeros to represent the initial hidden states of a
        batch of sequences.
...
class ScaledDotAttention(nn.Module):
    def __init__(self, hidden_size):
        ...
        # 1 / sqrt(d)
        self.scaling_factor = torch.rsqrt(torch.tensor(self.hidden_size,
                                                       dtype=torch.float))
    ...
    def forward(self, queries, keys, values):
        ...
        Arguments:
            queries: The current decoder hidden state.
            (batch_size x decoder_seq_len x hidden_size)
        ...
        Returns:
            context: weighted average of the values
            (batch_size x decoder_seq_len x hidden_size)
            attention_weights: Normalized attention weights for each encoder
            hidden state. (batch_size x decoder_seq_len x encoder_seq_len)

            The output must be a softmax weighting over the encoder_seq_len
            annotations.
        """
        # TODO: Q1
        # ------------
        # FILL THIS IN
        # ------------
        q: torch.Tensor = self.Q(queries)
        k: torch.Tensor = self.K(keys)
        v: torch.Tensor = self.V(values)
        unnormalized_attention: torch.Tensor = torch.bmm(q, k.transpose(1, 2)) * self.scaling_factor
        attention_weights: torch.Tensor = self.softmax(unnormalized_attention)
        context: torch.Tensor = torch.bmm(attention_weights, v)
...
class CausalScaledDotAttention(nn.Module):
    ...
    def forward(self, queries, keys, values):
        ...
        Arguments:
            ...
            keys: The current decoder hidden state.
            (batch_size x seq_len x hidden_size)
            values: The current decoder hidden state.
            (batch_size x seq_len x hidden_size)

        Returns:
            context: weighted average of the values
            (batch_size x seq_len x hidden_size)
            attention_weights: Normalized attention weights for each decoder
            hidden state. (batch_size x seq_len x seq_len)
        ...
        mask = torch.triu(self.neg_inf * torch.ones(unnormalized_attention.size()), diagonal=1)
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, inputs, annotations, hidden_init):
        ...
        Returns:
            ...
            annotations (batch_size x decoder_seq_len x encoder_seq_len)
        """
        batch_size, seq_len = inputs.size()
        # batch_size x decoder_seq_len x hidden_size
...
if __name__ == '__main__':
    ...
    args = AttrDict()
    args_dict = {'cuda': False, # True for GPU, False for CPU
                 'nepochs': 100,
    ...
    if TRAIN_MODEL:
        ...
        translated = translate_sentence(TEST_SENTENCE, transformer_encoder,
                                        transformer_decoder, None, args)
